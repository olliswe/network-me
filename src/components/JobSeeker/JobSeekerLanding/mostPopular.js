...
const MostPopularPanel = (props) => {
    ...
    const getMostPopularJobs = () => {
        setJobsRequest({loading:true});
        let headers = {"Content-Type": "application/json",
        "Authorization":`Token ${props.token}`
        };
        fetch(API+'jobseeker/most_popular', {headers, method:"GET"})
        .then(res => res.json())
        .then(data => {
...
export default connect(mapStateToProps)(MostPopularPanel)
